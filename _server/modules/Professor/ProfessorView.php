<?php
    require_once ('_server/modules/Professor/ProfessorHandler.php');
...
?>
        ...
        <table width="99%">
            <?php 
            $res = ProfessorHandler::ReqData_ProfessorList($search_key,$search_val);
            $firstLine = "<thead>";
            foreach($res->data as $key=>$row) {
                ...
                $szLine.='<td>';
                $szLine.='<button class="btnSelect" id="'.$row['fa_uid'].'">Select</button>';
                $szLine.='</td>';
                echo $szLine.='</tr>';
            }; ?>
        </table> </div>
